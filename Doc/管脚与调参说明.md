...
90°转弯由状态机分段执行。达到 `MAZE_TURN_MIN_MS` 后，对应侧红外连续 `MAZE_IR_CONFIRM_COUNT` 次检测到墙就退出；如果一直没有确认，则达到 `MAZE_TURN_MAX_MS` 后兜底退出。转弯期间若左右红外都检测不到墙且前方安全，会用 `MOTOR_TURN_LOST_PWM` 前进 `MAZE_TURN_LOST_FORWARD_MS` 做补位，然后继续原转弯状态。想让动作更细腻，可减小 `MAZE_TURN_STEP_MS`；转弯不足可增大 `MAZE_TURN_MAX_MS` 或略微提高 `MOTOR_TURN_OUTER_PWM`。掉头由 `MOTOR_TURN_BACK_SPIN_PWM`、`MAZE_TURN_BACK_STEP_MS` 和 `MAZE_TURN_BACK_CHECK_PAUSE_MS` 调节动作细腻程度；达到 `MAZE_TURN_BACK_MIN_MS` 后，当前方超声波距离连续大于 `FRONT_TURN_BACK_CLEAR_CM` 时提前退出，`MAZE_TURN_BACK_MAX_MS` 是最大兜底时间。
